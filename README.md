...
- `game/` — Godot project and WebGL runtime
- `game/webgl/` — integrated SR3 WebGL source and browser build
...
> Для AI-агентов ZIP используется как reference build / runtime artifact для анализа WebGL-архитектуры, тестирования, портирования и дальнейшей итерации SR3. Существующий `game/` не заменяется: WebGL живёт параллельно как браузерный runtime.

## WebGL Integration

- `game/webgl/src/` — исходный ES6 WebGL engine и игровые системы из архива.
- `game/webgl/dist/` — браузерный runtime/build.
- `game/webgl/package.json` — локальный WebGL toolchain.
- `docs/WEBGL_INTEGRATION.md` — правила объединения WebGL с архитектурой репозитория.
- `.github/workflows/integrate-sr3-webgl.yml` — автоматическая интеграция reference ZIP без `node_modules` и вложенных дубликатов.
