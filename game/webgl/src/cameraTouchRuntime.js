...
export function installCameraTouchControls(win = globalThis?.window) {
  ...
  style.textContent = `
    ...
    #cameraTouchControls button{width:42px;height:42px;border:1px solid rgba(125,216,255,.55);border-radius:10px;background:rgba(5,12,28,.84);color:#d9efff;font-size:18px;box-shadow:0 0 12px rgba(80,150,255,.18);touch-action:manipulation}
  ...
  root.innerHTML = `
    <span></span><button type="button" data-pan="up" aria-label="Камера вверх">▲</button><span></span>
    <button type="button" data-pan="left" aria-label="Камера влево">◀</button><button type="button" id="cameraFollowBtn" aria-label="Вернуть камеру к кораблю">◎</button><button type="button" data-pan="right" aria-label="Камера вправо">▶</button>
    <span></span><button type="button" data-pan="down" aria-label="Камера вниз">▼</button><span></span>
